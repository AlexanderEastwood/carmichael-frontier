...
LAG="19,31,41,43,47,53,59,61:1 19,23,31,37,41,43,53,59:1 19,31,37,41,43,47,53,59:1 19,31,37,41,43,47,59,61:1 19,29,31,37,41,43,47,53:1"
for p in 19,31,41,43,47,53,59,61 19,23,31,37,41,43,53,59 19,31,37,41,43,47,53,59 19,31,37,41,43,47,59,61 19,29,31,37,41,43,47,53; do
  for q in $(pgrep -f "[p]refix $p\$"); do kill -9 "$q"; done; rm -f "$JD/$(echo $p|tr , _).out"; done
rm -f "$JD/DONE"; sleep 2
python3 sub_gen.py $LAG > "$JD/SUBJOBS5"
echo "subjobs5=$(wc -l < "$JD/SUBJOBS5") start=$(date -u +%FT%TZ)" >> "$JD/PROGRESS"
run_one(){ f="$JD/$(echo "$1"|tr , _).out"; DYNT_KMAX="$KMAX" ./cm_dynt 64 "$B" --prefix "$1" > "$f" 2>&1; }
export -f run_one; export B JD KMAX
xargs -P 28 -I{} bash -c "run_one \"\$@\"" _ {} < "$JD/SUBJOBS5"
