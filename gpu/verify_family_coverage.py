...
usage: python3 verify_family_coverage.py <census.json> <records dir> [<expected bound U>]
...
def main() -> None:
    ...
    U_exp = int(sys.argv[3]) if len(sys.argv) > 3 else None
    ...
    for row in fam:
        ...
        if int(m.group(6)) != rmax: problems.append(f"D={D}: header rmax {m.group(6)} != census {rmax}")
    ...
    print(f"family={os.path.basename(fam_path)} moduli={len(fam)} exchange_instances_expected={radii_total} problems={len(problems)} hits={len(hits)}")
